...
namespace Sample
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            NLog.Extensions.Logging.NLogLoggerFactory nLogLoggerFactory = new NLog.Extensions.Logging.NLogLoggerFactory();
            ILogger<SmsRuProvider> logger = nLogLoggerFactory.CreateLogger<SmsRuProvider>();


            var configuration = new SmsRuConfiguration(
                "",
                "",
                "",
                "",
            ...
            ISmsProvider sms = new SmsRuProvider(configuration, logger);

            Console.WriteLine("Метод Send:");
            Console.WriteLine(sms.Send(configuration.SmsNumberFrom, new string[] { "79117693358", "79117693358" }, DateTime.Now.ToLongTimeString(), EnumAuthenticationTypes.Simple));
            Console.WriteLine(sms.Send(configuration.SmsNumberFrom, "79117693358", DateTime.Now.ToLongTimeString()));
            Console.WriteLine(sms.Send(configuration.SmsNumberFrom, new string[] { "79117693358", "79117693358" }, DateTime.Now.ToLongTimeString()));
